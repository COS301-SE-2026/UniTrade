...
import type { ProposeMeetupPayload, Review } from "../../types/listing";
...
export function seedMockTransaction(
...
export const orderFlowHandlers = [
  http.get(
    "http://localhost:5000/api/reservations/:id/transaction-status",
    ({ params }) => {
...
export const listingLifecycleHandlers = [
  http.post("http://localhost:5000/api/listings", async ({ request }) => {
    const body = (await request.json()) as Omit<
      MockListing,
      "listingId" | "createdAt" | "viewCount" | "seller" | "sellerId"
    >;
...
export const browseAndReserveHandlers = [
  ...
  http.post("http://localhost:5000/api/reservations", async ({ request }) => {
    ...
    const reservation: ReservationListItem = {
      ...
      completedAt: null,
      counterParty: {
        userId: listing?.sellerId ?? "seller-1",
        name: "Test Seller",
        initials: "TS",
      },
      listing: {
...
export const chatHandlers = [
  http.get(
    "http://localhost:5000/api/reservations/:id/messages",
    ({ params }) => {
      const items = mockMessages.filter((m) => m.reservationId === params.id);
      return HttpResponse.json({
        items,
        hasMore: false,
        oldestMessageId: items[0]?.messageId ?? null,
      });
    },
  ),
...
export const meetupHandlers = [
  http.get("https://nominatim.openstreetmap.org/reverse", () => {
    return HttpResponse.json({
      display_name: "Merensky Library, Lynnwood Road, Pretoria",
    });
  }),

  http.post(
    "http://localhost:5000/api/reservations/:id/meetup/propose",
    async ({ params, request }) => {
