...
// Mock useSearchQuery to avoid router context issues
...
// Fixed import path for SummaryCard
vi.mock('../../pages/buyer/Reservation', () => ({
  SummaryCard: (props: { label: string; value: string }) => (
...
// Fixed import path for ReviewModal
...
it('shows an error message and retries on button click', async () => {
  vi.mocked(listingsService.getCompletedSales)
...
it('falls back to a generic error message for a non-Error rejection', async () => {
  ...
  renderMySales();
  expect(
    await screen.findByText('An error occurred while loading your sales.')
  ).toBeInTheDocument();
});
