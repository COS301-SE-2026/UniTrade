...
export class RealHubConnection implements IHubConnection {
  ...
  constructor() {
    ...
    const tokenUrl = `${origin}/api/auth/hub-token`;

    this.connection = new signalR.HubConnectionBuilder()
      .withUrl(hubUrl, {
        withCredentials: true,
        accessTokenFactory: async () => {
          const results = await fetch(tokenUrl, { credentials: "include" });
          if (!results.ok) {
            throw new Error("hub token fetch failed");
          }
