...
import { useBrowsingListings } from "../../hooks/useBrowseListings";
...
function ListingCard({
  ...
  const handleReserve = async (e: React.MouseEvent) => {
    ...
    if (result.success) {
      ...
      //showToast('success', 'Sucessfully reserved| Redirecting to your reservations....'); i kinda don't like this .. like this
...
export default function BrowseAllListing() {
  const navigate = useNavigate();
  const { data, isLoading, error } = useBrowsingListings();
  ...
  if (error)
    return (
      <div className="flex items-center justify-center h-64">
        <p className="text-sm text-red-400">{error instanceof Error ? error.message : 'Failed to listings'}</p>
      </div>
    );
