...
export default function AdminListingQueue() {
  ...
  const decision = useMutation({
    mutationFn: (vars: { id: string; action: 'approve' | 'remove'; reason?: string }) =>
  ...
  const filteredRows = listings.filter((l) => {
    ...
    const matchSearch = l.title.toLowerCase().includes(q) || l.sellerInitials.toLowerCase().includes(q)
  ...
  if (error) {
    return <p className="text-sm text-red-600">Failed to load flagged listings</p>
  }
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-gray-100 text-xs">
            {sortedRows.length === 0 ? (
              ...
              sortedRows.map((l) => (
                <tr key={l.listingId} className="hover:bg-gray-50/50 transition-colors">
                  ...
                  <td className="py-4 px-4 text-center">
                    ...
                    <div className="mt-1 text-[10px] text-gray-500">Score {l.riskScore}/100</div>
                    <div className="w-full max-w-[120px] mx-auto mt-1 bg-gray-200 rounded-full h-1.5">
                      <div
                        ...
                        style={{ width: `${Math.min(100, Math.max(0, l.riskScore))}%` }}
                      />
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-4 text-center">
                    {/*{l.imageMatchScore === null ? (
                      ...
                      <span
                        className={`inline-flex items-center gap-1 font-semibold ${isLowMatch(l) ? 'text-rose-700' : 'text-emerald-700'
                          }`}
                      >
      ...
      {removeTarget && (
        ...
        >
          <div className="w-full max-w-md bg-white dark:bg-navy-800 rounded-xl p-6 space-y-4">
            <div>
              <h2 id="remove-listing-title" className="text-base font-bold text-gray-900 dark:text-white">
            ...
            <div>
              <label htmlFor="remove-reason" className="text-xs font-medium text-gray-700">
                Reason
              </label>
              <textarea
                id="remove-reason"
