...
const confirmModalToneClasses: Record<
...
export function Panel({
  ...
  return (
    <div
      className={`bg-white dark:bg-navy-800 rounded-xl border border-gray-200 dark:border-white/10 p-5 ${className}`}
    >
...
export function InfoRow({
  ...
  return (
    <div className="flex items-center justify-between py-1.5 text-sm">
      ...
      <span className="text-navy-700 dark:text-white font-medium text-right">
        {value}
      </span>
    </div>
  );
...
export function StatusBadge({
  ...
  return (
    <span
      className={`text-xs font-semibold px-3 py-1 rounded-full whitespace-nowrap ${toneClasses[tone]}`}
    >
      {label}
    </span>
  );
...
export function StarRating({
  value,
  size = 16,
}: Readonly<{ value: number; size?: number }>) {
    ...
    <Panel title={title}>
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-gray-600">{person.faculty}</p>
          <div className="mt-1">
      ...
      {!loading && notes.length === 0 && (
        <div className="flex flex-col items-center justify-center text-center py-6 mb-3">
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Add an internal note to keep a record for this case.
          </p>
        </div>
      )}
