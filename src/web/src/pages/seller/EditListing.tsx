        ...
        <div className="relative">
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            <div className="w-44 pt-2 shrink-0">
              <h4 className="text-sm font-bold text-slate-900">Step 1: Basic Information</h4>
            </div>
            <div className="flex-1 w-full bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className={formData.category !== 'Other' ? 'md:col-span-2' : 'md:col-span-3'}>
                ...
                {formData.category === 'Textbook' && (
                  <div>
                    <select value={formData.moduleTag} onChange={e => handleChange('moduleTag', e.target.value)}
                ...
                {formData.category === 'Electronics' && (
                  <div>
                    <input type="text" value={formData.customField}
          ...
          <div className="flex flex-col md:flex-row gap-6 items-start">
            ...
            <div className="flex-1 w-full bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-4 gap-4">
                ...
                {Array.from({ length: Math.max(0, 4 - formData.images.length) }).map((_, idx) => (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    {(['Like_New', 'Good', 'Fair', 'Worn'] as const).map(item => (
                      <button key={item} type="button" onClick={() => handleChange('condition', item)}
            ...
            <div className="flex-1 w-full bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
              ...
              <div className="flex gap-4 items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                <div className="w-10 h-12 rounded overflow-hidden flex-shrink-0">
                  <img
                    src={formData.images[0] || biologyTextbook}
                    alt={formData.title}
                    className="w-full h-full object-cover"
                  />
                </div>
                <div className="overflow-hidden">
                  <h5 className="text-sm font-bold text-slate-800 truncate">{formData.title || 'Untitled Listing'}</h5>
                  <p className="text-xs text-slate-400">Changes Preview Summary</p>
                </div>
              </div>
            </div>
          </div>
