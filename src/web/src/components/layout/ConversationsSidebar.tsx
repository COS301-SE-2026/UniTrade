...
export default function ConversationsSidebar({ role }: Readonly<{ role: 'buyer' | 'seller' }>) {
    ...
    const active = reservations
        ...
        .filter((r: ReservationListItem) => {
            ...
            const mainListing = r.listings[0];
            const haystack = [
                r.counterParty.name,
                mainListing?.title ?? '',
                ...
                {active.map((r: ReservationListItem) => {
                    ...
                    return (
                        <NavLink
                            ...
                            className={`flex items-center gap-3 px-4 py-3 border-b border-gray-50 hover:bg-gray-50 transition-colors ${isActive ? 'bg-gray-200' : ''
                                }`}
                        >
                            <div className="relative shrink-0">
                                ...
                                {primaryListing?.imagePath && (
                                    <img
                                        src={`${apiOrigin}${primaryListing.imagePath}`}
