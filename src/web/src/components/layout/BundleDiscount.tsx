...
export function BundleDiscountCard({
 ...
 if(data!== prevData) {
    ...
    `${data!.percent}% off when a buyer reserves ${data!.minItems}+ of your listings (Reserve within budget only).`
    : liveCount
    ...
    return(
        <div className="bg-white rounded-xl border border-gray-200 p-4">
            <div className="flex items-start gap-3">
                <span className="w-9 h-9 rounded-full bg-nay-50 text-navy-700 flex items-center justify-center shrink-0">
                    <IconTag size={18} />
                </span>
            ...
            {compact && (
                ...
                type="button"
                onClick={() => setOpen((o) => !o)}
                className="text-xs font-semibold px-3 py-1.5 rounded-full border border-gray-300 text-navy-700 ho ver:bg-gray-50">
